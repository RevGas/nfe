...
class WSLoteConsulta {
    ...
    NFLoteConsultaRetorno consultaLote(final String numeroRecibo, final DFModelo modelo) throws Exception {
        return new Persister(new DFRegistryMatcher(), new Format(0)).read(NFLoteConsultaRetorno.class, efetuaConsulta(gerarDadosConsulta(numeroRecibo).toString(), modelo));
    }
