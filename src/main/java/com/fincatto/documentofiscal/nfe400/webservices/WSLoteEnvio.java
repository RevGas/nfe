...
import br.inf.portalfiscal.nfe.wsdl.nfeautorizacao3.pr.NfeCabecMsg;
import br.inf.portalfiscal.nfe.wsdl.nfeautorizacao4.svrs.NFeAutorizacao4;
import br.inf.portalfiscal.nfe.wsdl.nfeautorizacao4.svrs.NFeAutorizacao4Soap;
import br.inf.portalfiscal.nfe.wsdl.nfeautorizacao4.svrs.NfeResultMsg;
    ...
    private TRetEnviNFe getTRetEnviNFe(final String endpoint, final DFUnidadeFederativa uf, String loteAssinadoXml) throws MalformedURLException, JAXBException, Exception {
        ...
        switch (uf) {
            case PI :
                ...
                JAXBContext contextSVRS = JAXBContext.newInstance("br.inf.portalfiscal.nfe");

                Unmarshaller jaxbUnmarshallerSVRS = contextSVRS.createUnmarshaller();
                StringReader readerSVRS = new StringReader(loteAssinadoXml);
                JAXBElement<TEnviNFe> tEnviNFeSVRS = (JAXBElement<TEnviNFe>) jaxbUnmarshallerSVRS.unmarshal(readerSVRS);

                final br.inf.portalfiscal.nfe.wsdl.nfeautorizacao4.svrs.NfeDadosMsg nfeDadosMsgSVRS = new br.inf.portalfiscal.nfe.wsdl.nfeautorizacao4.svrs.NfeDadosMsg();
                nfeDadosMsgSVRS.getContent().add(tEnviNFeSVRS);

                NFeAutorizacao4Soap portSVRS = new NFeAutorizacao4().getNFeAutorizacao4Soap();
                NfeResultMsg resultSVRS = portSVRS.nfeAutorizacaoLote(nfeDadosMsgSVRS);
                return ((JAXBElement<TRetEnviNFe>) resultSVRS.getContent().get(0)).getValue();
            case PR :
